...
\item We quantify how the transferability gap grows as the similarity between base
task and target task decreases.
\item We further compare both frozen and fine-tuned transfer to using random filters, which we find not to work as well on the large ImageNet dataset as they have been reported to \citep{Jarrett-ICCV2009} for smaller datasets (Section~\ref{sec:experiments_random_nm}).
\item Finally, we find an interesting effect where fine-tuning filters from a different dataset can lead to better generalization, even when the datasets are the same size and training time is identical (Section~\ref{sec:experiments_ab}).
\end{enumerate}
...
We have noted the curious tendency of Gabor features to show up in the first layer of neural networks trained on natural images for a large range of network architectures and cost functions. We say these features are general if they are useful for many different tasks or specific if they are useful only for a few. We can measure the specificity of a set of features by training the set on one task \dA and then using the feature set for another task \dB.
Of course, this definition depends on the similarity between \dA and \dB. In this paper we take tasks \dA and \dB to be classifying different non-overlapping subsets of a particular dataset.
These subsets can be chosen in particular ways so that they are as similar to or as different from each other as possible.
...
%HERE\emph{Third row:} In this control, the weights (filters) of all layers are initialized randomly (checkered pattern). The weights of the first $N$ layers of the network are locked, meaning they do not change during training. The remaining layers are trained as normal on half the dataset. A replicate (not pictured) is performed on the other half of the dataset.
